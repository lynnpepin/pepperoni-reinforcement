...
def in_circles(c,Cir):
    ...
    (CircleVertex, CircleVertex_Collection) -> Bool
    """
    for ci in Cir:
        if c.index == ci.index:
            yesorno = True
...
def generate_circlepacking():
 ...
 #       if j == len(cb):
  #          j = 1
    ...
    for i in range(0,n_c):
        if circles[i].index != LD[1].index: # fix the radius of the second leading dancer
...
def modify_circlepacking(dr,LD, AD):
    """
    list of float, list of CircleVertex, list of CircleVertex 
    dr: the changes added on each leading dancer
    LD: the collection of leading dancers
    AD: the collection of accompying dancers
    """
...
def generate_boundary_edges(LD, AD, circles, faces, cb, anchor_x, anchor_y, cb_origin):
    """
    list of CircleVertex, list of CircleVertex, list of CircleVertex, list of Face, 
    CircleVertex, list of CircleVertex, list of CircleVertex, CircleVertex 
    -> nx5 array, list of float, list of float
    return boundary_edges, r_lb, r
    r_lb: list of radii of leading dancers
    r: list of radii of all circles
    """
    #Layout circles
    faces_copy = faces.copy()
    #layout anchers along x
